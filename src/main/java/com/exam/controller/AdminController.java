...
@Controller
@SessionAttributes(names = {"empDTO","wageDTO","allAttList"})
public class AdminController {
   ...
   public ModelAndView searchEmp(@RequestParam("name") String name,ModelMap m) {
      ...
      if (empDTO != null) { //조회한 직원이 존재하는 경우
         wageDTO=wageService.paycheck(empDTO.getEmp_id()); //조회한 이름과 동일한 직원의 급여 정보를 wageDTO에 저장
         ...
         System.out.println(emp_id);
   	     List<AttendanceDTO> allAttList = attendanceService.allAtt(emp_id);
         mav.addObject("allAttList", allAttList);
